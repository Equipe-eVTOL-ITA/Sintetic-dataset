...
class Cam( ):
    ...
    def intrinsic_matrix(self):
        ...
        my = self.y_resolution/(self.sensor_height*aspect_ratio_y)*scale
    ...
    def correct_distortion(self, point, k1 = 0.0, k2 = 0.0, k3 = 0.0, axis = None):

        r = length(point[0], point[1]) if axis is None else point[0] if axis is 'x' else point[1]

        dr = k1*r**3 + k2*r**5 + k3*r**7

        x = point[0]*(1 + dr/r) if axis is None else point[0]*(1 + dr/r) if axis is 'x' else point[0]
        y = point[1]*(1 + dr/r) if axis is None else point[1] if axis is 'x' else point[1]*(1 + dr/r)

        return Vector([x, y, point[2]])
...
class Image_object:
    ...
    def to_image_coord(self, point):
        ...
        point_camera_coord_corrected = self.camera.correct_distortion(point_camera_coord, k1 = -0.12, k2 = 0.03, k3 = 0.01)

        point_image_coord = self.K @ point_camera_coord_corrected
