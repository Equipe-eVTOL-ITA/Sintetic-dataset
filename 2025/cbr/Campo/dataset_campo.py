...
for i in range(set_len):
    ...
    camera.rotation_euler = (np.random.uniform(-np.pi/36, np.pi/36), np.random.uniform(-np.pi/36, np.pi/36), np.random.uniform(-np.pi, np.pi))
    ...
    with open(labels_file_name + str(i) + '.txt', 'a') as file:
        ...
        bbox = image_balde.get_bounding_box( )
