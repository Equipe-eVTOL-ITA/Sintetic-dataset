...
for i in range(set_len):
    ...
    light.data.energy = np.random.uniform(0.5*light_scale, 4*light_scale)
    light.data.color = colorsys.hsv_to_rgb(np.random.uniform(0, 1), np.random.uniform(0, 0.8), 1)

    camera.location = (balde_center[0] + np.random.uniform(-radius, radius), balde_center[1] + np.random.uniform(-radius, radius), np.random.uniform(2, 7))
    drone.location = camera.location
    camera.rotation_euler = (np.pi/4 + np.random.uniform(-np.pi/36, np.pi/36), np.random.uniform(-np.pi/36, np.pi/36), np.random.uniform(-np.pi, np.pi))
    drone.rotation_euler = camera.rotation_euler
    camera.data.lens = np.random.random_integers(40, 50)
    ...
    with open(labels_file_name + str(i) + '.txt', 'a') as file:
        ...
        bbox_balde = image_balde.get_bounding_box( )

        image_tampo = blender.Image_object(tampo, cam)
        image_tampo.set_bounding_box( )
        bbox_tampo = image_tampo.get_bounding_box( )
